...
typealias NumberOfTrailsMap = MutableList<MutableList<Int?>>
...
fun TopoMap.pointsOfHeight(height: Int): Sequence<Pair<Int, Int>> =
    sequence {
        yieldAll(allCells().filter { (x, y) -> get(x)[y] == height })
    }

fun TopoMap.neighborsOneHigher(coord: Coord): Sequence<Pair<Int, Int>> =
    sequence {
        val (x, y) = coord
...
fun TopoMap.numberOfTrails(): NumberOfTrailsMap {
    val rows = size
    val cols = get(0).size

    var numberOfTrailsMap: NumberOfTrailsMap =
        (0..<rows)
            .map { x ->
                (0..<cols).map { y ->
                    if (cell(x to y) == 9) 1 else null
                }.toMutableList()
            }.toMutableList()

    for (height in 8 downTo 0) {
        pointsOfHeight(height).forEach { (x, y) ->
            numberOfTrailsMap[x][y] =
                neighborsOneHigher(x to y)
                    .map { (nx, ny) -> numberOfTrailsMap[nx][ny] }
                    .filterNotNull()
                    .sum()
        }
    }

    return numberOfTrailsMap
}
...
object Day10 {
    ...
    fun part2(input: String): Int {
        val topoMap = parse(input)
        val numberOfTrails = topoMap.numberOfTrails()
        return topoMap.pointsOfHeight(0).map { (x, y) ->
            numberOfTrails[x][y]
        }.filterNotNull().sum()
    }
